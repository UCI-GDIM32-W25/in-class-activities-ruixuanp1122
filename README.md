...
In this assignment, I worked on implementing the core gameplay mechanics for a simple 2D infinite runner game in Unity. I focused on setting up the player’s jump behavior using physics, including detecting when the player is grounded so that jumping is only possible while on the ground. 
I also configured the ground and collision system to ensure the player interacts correctly with the environment. Through this process, I learned how colliders, Rigidbody2D, and ground checks work together to control movement and prevent unintended behavior such as falling through the ground or double jumping.
